...
export class AllInboxesComponent implements OnInit, OnDestroy {
  ...
  advanceTodo(todo: any): void {
    this.api.patch<any>(`/todos/${todo.id}/advance`, {}).subscribe((updated) => this.mergeTodo(updated));
  }

  completeTodo(todo: any): void {
    this.api.patch<any>(`/todos/${todo.id}/complete`, {}).subscribe((updated) => this.mergeTodo(updated));
  }

  todoAssigned(updated: any): void { this.mergeTodo(updated); }
